...
#增加特征的维度，并捕获更复杂的特征,经证实可有效抑制错位和噪点生成
...
def reconstruction(inps,addition):
    reference = inps[:,:,:,-64:]
    # 改进，使用DRDB模块替换卷积层
    # 定义 nDenselayer 为 3，growthRate 为 16，那么 DRDB 模块将会包含 3 个 make_dilation_dense 模块，每个模块将通道数增加 16。
    # 这意味着输入通道数为 nChannels，输出通道数将是 nChannels + 3 * growthRate。
    x = DRDB(64, nDenselayer=3, growthRate=16)(inps)
    # = tf.keras.layers.SeparableConv2D(64,(3,3), padding = 'same')(inps) #(256,256)
    ...
    x = DRDB(128, nDenselayer=3, growthRate=16)(concat)
    ...
    x = DRDB(256,nDenselayer=3,growthRate=16)(concat)
