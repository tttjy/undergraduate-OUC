...
function user_stop(obj, id) {
  layer.confirm('确认要停用吗？', function (index) {
    ...
    $(obj)
      .parents('tr')
      .find('.user-status')
      .html('<span class="label">已停用</span>')
    $(obj).remove()
    layer.msg('已停用!', 1)
  })
}
...
function user_start(obj, id) {
  layer.confirm('确认要查看吗？', function (index) {
    $(obj)
      ...
      .find('.user-manage')
      .prepend(
        '<a style="text-decoration:none" onClick="user_stop(this,\'10001\')" href="javascript:;" title="查看"><i class="icon-hand-down"></i></a>'
      )
    $(obj)
      .parents('tr')
      .find('.user-status')
      .html('<span class="label label-success">查看</span>')
    $(obj).remove()
    layer.msg('已启用!', 1)
  })
}
...
function article_xiajia(obj, id) {
  $(obj)
    ...
    .find('.article-manage')
    .prepend(
      '<a style="text-decoration:none" onClick="article_fabu(this,\'10001\')" href="javascript:;" title="发布"><i class="icon-hand-up"></i></a>'
    )
  $(obj)
    .parents('tr')
    .find('.article-status')
    .html('<span class="label">已下架</span>')
  $(obj).remove()
}
/*资讯-发布*/
function article_fabu(obj, id) {
  ...
  $(obj)
    .parents('tr')
    .find('.article-status')
    .html('<span class="label label-success">已发布</span>')
  $(obj).remove()
}
/*管理员-删除*/
