      ...
      >
        ...
        <table class="table table-bordered table-hover table-striped">
          ...
          <tr>
            ...
            <td>
              <input type="radio" name="type" value="1" ${1 eq
              user.type?'checked':'' }>用户管理员 <input type="radio"
              name="type" value="2" ${2 eq user.type?'checked':'' }>题库管理员
              <input type="radio" name="type" value="3" ${3 eq
              user.type?'checked':'' }>题库使用者
            </td>
          </tr>
          <tr>
            <td class="head">用户状态</td>
            <td>
              <input type="radio" name="status" value="0" ${0 eq
              user.status?'checked':'' }>禁用 <input type="radio" name="status"
              value="1" ${1 eq user.status?'checked':'' }>正常
            </td>
          </tr>
          <tr>
            <td colspan="2" class="edit">
              <input type="submit" class="btn btn-warning" value="保存" /><input
