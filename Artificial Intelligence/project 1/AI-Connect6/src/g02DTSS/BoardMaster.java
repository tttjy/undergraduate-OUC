...
import static g02DTSS.Road._FORWARD;

/*着法：1.我方四字或五子findwinMoves; 2.一般情况下寻找好点findGenerateMoves
* 搜索方法：1.单胁迫findSingleBlocks; 2.双胁迫findDoubleBlocks; 3.三胁迫findTripleBlocks；
* 计算迫着：我方收到的威胁数countAllThreats*/
...
//
//        int chooseNum = Math.min(POINTSWIDTH, pointslist.size() / 2);
//
//        for (int i = 0; i < chooseNum; i++) {
//            Node node = pointslist.get(i);
//
//            for (int j = i + 1; j < chooseNum; j++) {
//                Node nextNode = pointslist.get(j);
//                MoveMaster move = new MoveMaster(node.getPos(), pointslist.get(i).getPos());
//                move.setScore(RoadTable.evaluateChessStatus(whoseMove(), roadTable));
//                moves.add(move);
//            }
...
//    }
    ...
    ArrayList<MoveMaster> findSingleBlocks(){
        ...
        while(pointslist.size() > num){
            pointslist.remove(pointslist.size()-1);//路被堵住，移出路表
        }
...
//    public ArrayList<MoveMaster> findSingleBlocks() {
//        ArrayList<MoveMaster> movelist = new ArrayList<>();
//        initializeVariables(); // 提取初始化变量的逻辑
//
//        RoadList R1 = roadTable.getPlayerRoads()[0][4];
//        RoadList R2 = roadTable.getPlayerRoads()[0][5];
//
//        ArrayList<Road> four = new ArrayList<>(R1);
//        ArrayList<Road> five = new ArrayList<>(R2);
//
//        iterateAndCheckRoads(four);
//        iterateAndCheckRoads(five);
//
//        filterAndSortPoints();
//
//        updateRoadTable(movelist);
//
//        return generateMoveList(movelist);
//    }
//
...
//    }
//
//    private void iterateAndCheckRoads(ArrayList<Road> roads) {
//        for (Road road : roads) {
//            for (int i = 0; i < 6; i++) {
//                int pos = road.getStartPos() + i * _FORWARD[road.getDir()];
//                if (get(pos) == EMPTY && !vis[pos]) {
//                    processPoint(pos);
//                }
//            }
//        }
//    }
//
//    private void processPoint(int pos) {
//        changeRoads(pos);
//        if (roadTable.getPlayerRoads()[0][4].size() + roadTable.getPlayerRoads()[0][5].size() == 0) {
//            pointlist.add(pos);
//        }
//        unchangeRoads(pos);
//        vis[pos] = true;
//    }
//
//    private void filterAndSortPoints() {
//        pointslist.clear();
//        findblanks();
//        pointslist.sort(Node.scoreComparator);
//
//        int sum = pointslist.stream().mapToInt(Node::getScore).sum();
//        int average = sum / pointslist.size();
//
//        int num = 0;
//        for (int i = 0; i < pointslist.size(); i++) {
...
//    }
//
//    private void updateRoadTable(ArrayList<MoveMaster> movelist) {
//        for (Integer point : pointlist) {
//            for (Node points : pointslist) {
//                if (point != points.getPos()) {
//                    processMoveAndUpdateRoadTable(point, points, movelist);
//                }
//            }
//        }
//    }
//
//    private void processMoveAndUpdateRoadTable(int point, Node points, ArrayList<MoveMaster> movelist) {
//        MoveMaster move = new MoveMaster(point, points.getPos());
//        changeRoads(move);
//        move.setScore(RoadTable.evaluateChessStatus(whoseMove(), roadTable));
//        unchangeRoads(move);
...
//    }
    ...
    private ArrayList<MoveMaster> generateMoveList(ArrayList<MoveMaster> movelist) {
        return movelist;
    }
...
//        initializeVisArray();
//
//        // Opponent's connected fours or fives
//        RoadList four = getOpponentRoadList(4);
//        RoadList five = getOpponentRoadList(5);
//
//        collectEmptyPoints(four);
//        collectEmptyPoints(five);
//
//        int minThreats = four.size() + five.size();
//
//        for (int i = 0; i < pointlist.size(); i++) {
//            for (int j = i + 1; j < pointlist.size(); j++) {
//                changeAndEvaluateThreats(pointlist.get(i), pointlist.get(j), movelist, minThreats, four, five);
//            }
//        }
//
//        return movelist;
//    }
//
//    private void initializeVisArray() {
//        for (int i = 0; i < SIDE * SIDE; i++) {
//            vis[i] = false;
//        }
//    }
//
//    private RoadList getOpponentRoadList(int length) {
//        return whoseMove() == WHITE ? roadTable.getPlayerRoads()[length][0] : roadTable.getPlayerRoads()[0][length];
//    }
//
//    private void collectEmptyPoints(RoadList roads) {
//        for (Road road : roads) {
//            for (int i = 0; i < 6; i++) {
//                int pos = road.getStartPos() + i * _FORWARD[road.getDir()];
//                if (get(pos) == EMPTY && !vis[pos]) {
...
//    }
//
//    private void changeAndEvaluateThreats(int point1, int point2, ArrayList<MoveMaster> movelist,
//                                          int minThreats, RoadList four, RoadList five) {
//        changeRoads(point1);
//        changeRoads(point2);
//
//        int nowThreats = four.size() + five.size();
...
//            movelist.clear();
//            minThreats = nowThreats;
//            movelist.add(new MoveMaster(point1, point2));
//        } else if (nowThreats == minThreats) {
//            movelist.add(new MoveMaster(point1, point2));
//        }
//
//        unchangeRoads(point1);
//        unchangeRoads(point2);
//    }
